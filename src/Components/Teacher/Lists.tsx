...
import { TEACHER_LIST, DELETE_TEACHER } from "@/Apollo/query/teacher/teacher";
...
const Lists = () => {
    ...
    //Apollo
    const { data } = useSuspenseQuery(TEACHER_LIST, { variables: { searchDto: pagination }, errorPolicy: "all" });
    const [mutate, { loading }] = useMutation(DELETE_TEACHER, {
        onCompleted: (data) => {
            toast.success(data.deleteTeacher.message);
            setConfirm(null);
        },
        onError: (error) => {
    ...
    const onDelete = (id: number | null) => {
        if (id) {
            mutate({ variables: { deleteTeacherId: id } });
        }
    }
    ...
    return (
        <div>
            <TableHeader
                ...
                data={data?.getTeachers.results?.map(item => ({ id: item.id, name: item.name })) as any[]}
                headers={["ID", "Name"]}
