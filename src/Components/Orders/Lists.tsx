...
const Lists = () => {
    ...
    //Apollo
    const { data } = useSuspenseQuery(GET_ORDERS, { variables: { searchDto: pagination }, errorPolicy: "all" });
    ...
    return (
        <div>
            <TableHeader
                ...
                path=""
                data={data?.getOrders.results?.map(item => ({ id: item.id })) as any[]}
                headers={["ID"]}
                        ...
                        {tableData.map((item, i) => {
                            ...
                            return (
                                <tr key={i}>
                                    ...
                                    <td className={classes}>
                                        <p className="font-semibold text-[15px]">
                                            Tk. {Number.isInteger(item.total_price) ? item.total_price : item.total_price.toFixed(2)}
                                        </p>
                                    </td>
                                    <td className={classes}>
