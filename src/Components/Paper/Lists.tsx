...
import { PAPER_LIST, DELETE_PAPER } from "@/Apollo/query/paper/paper";
...
const Lists = () => {
    ...
    //Apollo
    const { data } = useSuspenseQuery(PAPER_LIST, { variables: { searchDto: pagination }, errorPolicy: "all" });
    const [mutate, { loading }] = useMutation(DELETE_PAPER, {
        onCompleted: (data) => {
            toast.success(data.deletePaper.message);
            setConfirm(null);
        },
        onError: (error) => {
            toast.error(error.message)
        },
        refetchQueries: ["getPapers"],
    ...
    const onDelete = (id: number | null) => {
        if (id) {
            mutate({ variables: { deletePaperId: id } });
        }
    }
    ...
    return (
        <div>
            <TableHeader
                ...
                path="/research-paper/add-new"
                data={data?.getPapers.results?.map(item => ({ id: item.id, name: item.name })) as any[]}
                headers={["ID", "Name"]}
                                    ...
                                    <td className={classes}>
                                        <div className="flex gap-3 justify-center items-center">
                                            <Tippy content="Edit" className="bg-gray-600 text-white py-[3px] px-2.5 rounded-md text-sm">
                                                <Link href={`/research-paper/${item.slug}`}>
                                                    <RiFileEditFill />
                                                </Link>
                                            </Tippy>
