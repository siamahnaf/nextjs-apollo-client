...
import { INSTITUTION_LIST, DELETE_INSTITUTION } from "@/Apollo/query/institution/institution";
...
const Lists = () => {
    ...
    //Apollo
    const { data } = useSuspenseQuery(INSTITUTION_LIST, { variables: { searchDto: pagination }, errorPolicy: "all" });
    const [mutate, { loading }] = useMutation(DELETE_INSTITUTION, {
        onCompleted: (data) => {
            toast.success(data.deleteInstitution.message);
            setConfirm(null);
        },
        onError: (error) => {
    ...
    const onDelete = (id: number | null) => {
        if (id) {
            mutate({ variables: { deleteInstitutionId: id } });
        }
    }
    ...
    return (
        <div>
            <TableHeader
                ...
                path="/institution/add-new"
                data={data?.getInstitutions.results?.map(item => ({ id: item.id, name: item.name })) as any[]}
                headers={["ID", "Name"]}
            ...
            <Table
                ...
                data={data?.getInstitutions.results || []}
                                    ...
                                    <td className={classes}>
                                        <div className="flex gap-3 justify-center items-center">
                                            <Tippy content="Edit" className="bg-gray-600 text-white py-[3px] px-2.5 rounded-md text-sm">
                                                <Link href={`/institution/${item.slug}`}>
                                                    <RiFileEditFill />
                                                </Link>
                                            </Tippy>
            ...
            <TablePagination
                meta={data?.getInstitutions.meta}
