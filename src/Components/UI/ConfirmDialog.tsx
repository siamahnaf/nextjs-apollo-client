...
const ConfirmDialog = ({ isFetching = false, id, onConfirm, open, onClose, title = "Delete?", subtitle = "This action can't be undone!", icon = <MdDelete className="text-7xl mx-auto text-red-600" />, buttonText = "Delete" }: Props) => {
    return (
        <div>
            ...
            <div className={`fixed bg-white transition-all duration-300 z-50 left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[350px] shadow-2xl rounded-md px-5 py-7 ${open ? "opacity-100 -translate-y-1/2" : "opacity-0 -translate-y-[calc(50%+10px)]"}`}>
                {icon}
                <p className="mt-2 mb-1.5 font-semibold text-center text-red-600">{title}</p>
                <p className="font-medium text-center text-[15px] text-gray-500">{subtitle}</p>
